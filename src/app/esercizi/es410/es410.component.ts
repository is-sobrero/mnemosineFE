...
export class Es410Component implements OnInit {
  ...
  resolvePromise() {
    const result_0 = new Promise<string[]>((resolve, reject) => {
      this.connection
        .get('assets/exAssets/es410_dialoghi/es410_semplice.txt', {
          responseType: 'text',
        })
        .subscribe((data) => {
          resolve(data.split('\n'));
    ...
    const result_1 = new Promise<string[]>((resolve, reject) => {
      this.connection
        .get('assets/exAssets/es410_dialoghi/es410_medio.txt', {
          responseType: 'text',
        })
        .subscribe((data) => {
          resolve(data.split('\n'));
    ...
    const result_2 = new Promise<string[]>((resolve, reject) => {
      this.connection
        .get('assets/exAssets/es410_dialoghi/es410_difficile.txt', {
          responseType: 'text',
        })
        .subscribe((data) => {
          resolve(data.split('\n'));
  ...
  setWord() {
    let random: number = Math.floor(
      Math.random() * this.list_of_words.at(this.level).length,
    );
  ...
  checkContent() {
    ...
    if (
      this.word_typed.toLocaleLowerCase() === this.missing_word.toLowerCase()
    ) {
