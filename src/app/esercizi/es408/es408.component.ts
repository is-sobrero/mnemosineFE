...
export class Es408Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.connection
      .get('assets/exAssets/sinonimi.txt', { responseType: 'text' })
      .subscribe((data) => {
        ...
        this.connection2
          .get('assets/exAssets/contrari.txt', { responseType: 'text' })
          .subscribe((data) => {
            this.dictionary = data.split('\n') as string[];
  ...
  setWord(): void {
    var rand: number = Math.floor(
      Math.random() * this.list_of_words.at(this.level - 1).length,
    );
  ...
  checkContent() {
    if (
      this.word_typed.toLowerCase() != this.display.toLowerCase() &&
      this.selectedArray.includes(this.word_typed.toLowerCase()) &&
      !this.inputCache.includes(this.word_typed.toLowerCase())
    ) {
      this.points += 1;
    } else {
      this.errors += 1;
