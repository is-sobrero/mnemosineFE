...
export class Es405Component implements OnInit {
  ...
  ngOnInit() {
    ...
    this.connection
      .get('assets/exAssets/60000_parole_italiane.txt', {
        responseType: 'text',
      })
      .subscribe((data) => {
  ...
  setWord(): void {
    let random: number = Math.floor(
      Math.random() * this.difficulty[this.level - 1].length,
    );
  ...
  checkContent() {
    ...
    if (
      this.list_of_words.includes(this.word_typed.toLocaleLowerCase()) &&
      !this.word_cache.includes(this.word_typed.toLocaleLowerCase())
    ) {
      this.points += 1;
